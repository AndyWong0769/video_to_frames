...
class VideoToFramesApp:
    ...
    def create_widgets(self):
        # Title
        title = tk.Label(self.root, text="🎬 Video to Frames Tool",
        ...
        tk.Label(row1, text="📹 Video:", width=14, anchor="e", bg="#f5f7fa",
        ...
        btn_browse_video = tk.Button(row1, text="Browse", command=self.browse_video,
        ...
        tk.Label(row2, text="📁 Output:", width=14, anchor="e", bg="#f5f7fa",
                 font=(self.FONT_UI, 10)).pack(side="left")
        entry_out = tk.Entry(row2, textvariable=self.output_dir,
                             font=(self.FONT_MONO, 9), bg="white", relief="solid", bd=1)
        entry_out.pack(side="left", fill="x", expand=True, padx=5)
        btn_browse_out = tk.Button(row2, text="Browse", command=self.browse_output_dir,
                                    bg="#ecf0f1", font=(self.FONT_UI, 9), width=6)
        btn_browse_out.pack(side="right")

        # 3. Frames per second — shortened to "Frames:" to prevent clipping
        row3 = tk.Frame(main_frame, bg="#f5f7fa")
        row3.pack(fill="x", pady=8)
        tk.Label(row3, text="⏱️ Frames:", width=14, anchor="e", bg="#f5f7fa",
                 font=(self.FONT_UI, 10)).pack(side="left")
        ...
        tk.Label(row3, text=" (frames/sec, e.g. 1 = 1 frame per second)",
                 bg="#f5f7fa", fg="#7f8c8d",
        ...
        tk.Label(row4, text="🖼️ Format:", width=14, anchor="e", bg="#f5f7fa",
                 font=(self.FONT_UI, 10)).pack(side="left")
        ...
        self.status_label = tk.Label(main_frame, text="Ready", bg="#f5f7fa", fg="#7f8c8d",
                                     font=(self.FONT_UI, 9))
    ...
    def browse_video(self):
        path = filedialog.askopenfilename(
            ...
            filetypes=[("Video files", "*.mp4 *.avi *.mov *.mkv *.flv *.wmv *.mts"),
                       ("All files", "*.*")]
        )
        if path:
    ...
    def start_conversion(self):
        ...
        if not self.ffmpeg_path or not os.path.exists(self.ffmpeg_path):
            messagebox.showerror("Error",
                ...
                "or placed in the same directory as this program.")
            return
    ...
    def extract_frames(self):
        ...
        if total_frames <= 0:
            ...
            self.status_label.config(text="Unable to estimate total frames, extraction continues...")
        else:
            self.progress_bar["mode"] = "determinate"
            self.progress_bar["value"] = 0
        ...
        try:
                ...
                if match:
                    ...
                    if total_frames > 0:
                        ...
                        self.status_label.config(
                            text=f"Extracted {frame_count} frames / ~{total_frames} total frames")
                    else:
                        self.status_label.config(
                            text=f"Extracted {frame_count} frames (total unknown)")
            ...
            if returncode == 0:
                self.status_label.config(
                    text=f"✅ Conversion complete! Extracted {frame_count} images", fg="#27ae60")
                messagebox.showinfo("Complete",
                    f"Successfully extracted {frame_count} frames.\nSaved to:\n{out_dir}")
            else:
                self.status_label.config(
                    text="❌ Conversion failed. Please check video or ffmpeg", fg="#e74c3c")
                messagebox.showerror("Error",
                    "ffmpeg execution error. Please ensure ffmpeg is available\nand the video file is not corrupted.")
